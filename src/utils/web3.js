...
const approveLoanContract = async (contractAddress, to, tokenId, time) => {
  ...
  try {
    ...
    await loanContract.methods
      .offerNFTForLoaning(contractAddress, tokenId, time)
...
const getCreditScore = async (userAddress) => {
  try {
    const score = await scoreContract.methods
      .calculateScore(userAddress)
      .call();
    return score;
  } catch (error) {
...
const offerNFTForLoaning = async (minter, tokenID, time) => {
  ...
  try {
    ...
    await nftContract.methods
      .approve(loanContractAddress, tokenID)
      .send({ from: accounts[0] });

    // Offer the NFT for loaning
    await loanContract.methods
      .offerNFTForLoaning(minter, tokenID, time)
      .send({ from: accounts[0] });
  } catch (error) {
...
const requestToBorrow = async (minter, tokenID) => {
  ...
  try {
    await loanContract.methods
      .requestToBorrow(minter, tokenID)
      .send({ from: accounts[0] });
  } catch (error) {
  ...
  try {
    await loanContract.methods
      .approveLoanRequest(
        ...
        web3.utils.toWei(collateral.toString(), "ether")
      )
      .send({ from: accounts[0] });
  } catch (error) {
...
async function getLendersActiveTokens(address) {
  try {
    const tokens = await loanContract.methods
      .lendersActiveTokens(address)
      .call();
    console.log(tokens);
    return tokens;
  } catch (error) {
...
async function fetchAmountToPay(minter, tokenID, query) {
  try {
    const result = await loanContract.methods
      .getAmountToPay(minter, tokenID, query)
      .call();
